defmodule RCON.PacketTest do
	...
	@client_exec_packet << byte_size(@client_exec_payload) :: 32-signed-integer-little >> <> @client_exec_payload
	...
	test "decode client exec packet" do
		assert {:ok, {:exec, 0, "hello", :client}} ==
			RCON.Packet.decode_payload(byte_size(@client_exec_payload), @client_exec_payload, :client)
	end

	test "decode server auth resp packet" do
		assert {:ok, {:auth_resp, 0, "", :server}} ==
			RCON.Packet.decode_payload(byte_size(@server_auth_resp_payload), @server_auth_resp_payload, :server)
	end
end